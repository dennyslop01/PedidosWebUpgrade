...
namespace PedidosWebUpgrade.Web.Controllers
{
    ...
    public class CestaController : Controller
    {
        ...
        [HttpGet()]
        public async Task<IActionResult> Detalle(int IdOrden, string IdCli, string IdSuc, string IdCat, int back = 0, int TipoPedido = 1)
        {
            CestaViewModel Modelo = new CestaViewModel();
            try
            {
                //PERMISOS DE USUARIO
                List<Menu> _Permisos = await new UsuarioRepository(_configVariables).ObtenerPermisos(int.Parse(HttpContext.Session.GetString("idusuario")), "Cesta/Detalle");
                ...
                Modelo.Condicion = await new CestaRepository(_configVariables).ObtenerCondiciones();
                Modelo.Descuento = await new CestaRepository(_configVariables).ObtenerPorcentajes();
                Modelo.Encabezado = await new PedidoRepository(_configVariables).ObtenerEncabezadoOrden(IdOrden, IdCli);
                Modelo.Encabezado.TotalDescuento = Modelo.Encabezado.TotalDescuento;
                List<Customer> _Clientes = new ClienteRepository(_configVariables).ObtenerClientes((string)HttpContext.Session.GetString("idvendedor"), IdSuc, 2).Result.Where(x => x.CustomerId == IdCli).ToList();
                Modelo.NombreCliente = _Clientes.FirstOrDefault().Name;
                ViewBag.DiasRequerido = _Clientes.FirstOrDefault().DiasRequerido;

                Modelo.Carriers = await new CestaRepository(_configVariables).ObtenerF0005("55", "CR");
                Modelo.PuertosDescargas = await new CestaRepository(_configVariables).ObtenerF0005("55", "PS");
                Modelo.IncosTerms = await new CestaRepository(_configVariables).ObtenerF0005("42", "FR");
                Modelo.Referencias01 = await new CestaRepository(_configVariables).ObtenerF0005("00", "00");
                Modelo.ModalidadTransporte = await new CestaRepository(_configVariables).ObtenerF0005("00", "TM");
                Modelo.ForwardingAgents = await new PedidoRepository(_configVariables).ConsultarForwardingAgent();
                Modelo.PaisesFacturacion = await new PedidoRepository(_configVariables).ConsultarPaisesFacturacion(IdCli);
                Modelo.CodigoCliente = IdCli;

                //datos del pedido
                List<Orders> pedidoViewModels = await new PedidoRepository(_configVariables).ConsultarPedidos("", "", "", IdOrden, "", Convert.ToInt32(HttpContext.Session.GetString("idusuario")));
                Modelo.Pedido = pedidoViewModels.FirstOrDefault();
                Modelo.Productos = await new CestaRepository(_configVariables).ObtenerProductosCesta(IdOrden, IdCli, Modelo.Pedido.ListaPrecio ?? string.Empty);
                ...
                Modelo.CondicionesDePago = new PedidoRepository(_configVariables).ConsultarCondicionesDePago().Result.Select(x => new ListaGeneral() { Codigo = x.Codigo.Trim(), Descripcion = x.Codigo.Trim() + " - " + x.Descripcion.Trim() }).ToList();
                Modelo.ShipTo = new ClienteRepository(_configVariables).ObtenerShipTo(null, null, 2, null).Result.Select(x => new ListaGeneral() { Codigo = x.CustomerId, Descripcion = x.CustomerId + " - " + x.Name }).ToList();
                Modelo.Almacenes = new AlmacenRepository(_configVariables).ConsultarAlmacenes(0).Result.Select(x => new ListaGeneral { Codigo = x.IdAlmacen, Descripcion = x.IdAlmacen + " - " + x.Descripcion }).ToList();
                ...
                {
                    //FECHAS ESTIMADAS
                    List<FechasEstimadas> _ListFechasEstimadas = await new CestaRepository(_configVariables).ObtenerFechasEstimadas(IdOrden);
                    ...
                    {
                        Modelo.AnnosCorrelativos = new PedidoRepository(_configVariables).ConsultarContadoresAnnoPais(Modelo.PaisesFacturacion.FirstOrDefault().Codigo).Result.Select(x => new ListaGeneral() { Codigo = x.IdTipo.ToString(), Descripcion = x.Descripcion.Trim() }).ToList();
                    }
                    else
                    {
                        //NUNCA DEBERIA ENTRAR AQUI
                        Modelo.AnnosCorrelativos = new PedidoRepository(_configVariables).ConsultarContadoresAnnoPais(null).Result.Select(x => new ListaGeneral() { Codigo = x.IdTipo.ToString(), Descripcion = x.Descripcion.Trim() }).ToList();
                    }
                }
                else
                {
                    ...
                    Modelo.CodigoAnnocorrelativo = Modelo.Pedido.CodigoAnnoCorrelativo;
                    Modelo.AnnosCorrelativos = new PedidoRepository(_configVariables).ConsultarContadoresAnnoPais(Modelo.Pedido.CodigoPaisFacturacion).Result.Select(x => new ListaGeneral() { Codigo = x.IdTipo.ToString(), Descripcion = x.Descripcion.Trim() }).ToList();
                }
        ...
        public JsonResult VaciarCesta(int IdOrden)
        {
            ...
            try
            {
                _result = new CestaRepository(_configVariables).VaciarCesta(IdOrden).Result;
        ...
        public JsonResult EliminarProductoCesta(int IdOrden, string CodProd, string CodCli)
        {
            ...
            try
            {
                _result = new CestaRepository(_configVariables).EliminarProducto(IdOrden, CodProd).Result;
                _Encabezado = new PedidoRepository(_configVariables).ObtenerEncabezadoOrden(IdOrden, CodCli).Result;
